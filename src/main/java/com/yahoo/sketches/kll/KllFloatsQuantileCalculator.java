...
final class KllFloatsQuantileCalculator {
  ...
  KllFloatsQuantileCalculator(final float[] items, final int[] levels, final int numLevels, final long n) {
    ...
    ItemsAuxiliary.convertToPrecedingCummulative(weights_);
  }

  float getQuantile(final double phi) {
    final long pos = ItemsAuxiliary.posOfPhi(phi, n_);
  ...
  private float approximatelyAnswerPositonalQuery(long pos) {
    assert pos >= 0;
    assert pos < n_;
    final int index = ItemsAuxiliary.chunkContainingPos(weights_, pos);
    return items_[index];
  }
